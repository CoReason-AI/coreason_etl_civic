...
def process_civic_tsv(file_stream: bytes, entity_type: str, source_id_col: str) -> Iterator[dict[str, Any]]:
    """
    ...
    # Determine unique IDs for mapping
    # We ensure source_id_col is cast to string
    df = df.with_columns(pl.col(source_id_col).cast(pl.Utf8).alias("string_source_id"))

    # Convert to dictionaries
    records = df.to_dicts()

    for row in records:
        source_id = row.pop("string_source_id")
        coreason_id = str(generate_coreason_id(source_id))
        yield {
